...
class GameBoard:
    ...
    def makeMove(self, from_coord : (int, int), to_coord : (int, int)) -> bool:
        ...
        in_check = self.in_danger((self.king.x, self.king.y))
        if in_check:
    ...
    def process_click_event(self, screen, click_coord : (int, int)):
        ...
        if self.selected_tile == clicked_tile and self.selected_tile not in self.player_prev_move: # unselect player piece
            ...
            self.selected_tile = None
        elif clicked_piece and clicked_piece.getColor() == self.orientation: # select player piece
            ...
            if self.selected_tile:
                ...
                if isinstance(selected_piece, King) and isinstance(clicked_piece, Rook) and self.castle_is_valid(clicked_piece):
                    self.castle(screen, clicked_piece)
    ...
    def process_opponent_move(self, screen, move : ((int, int), (int, int))):
        ...
        # check fo special case of castling
    ...
    def castle(self, screen, rook):
        ...
        if rook.x < self.king.x:
            ...
            self.draw_tile(screen, (0, 7))
            self.draw_tile(screen, (self.king.x + 2, 7))
        elif rook.x > self.king.x:
            self.board[7][self.king.x + 1] = rook
            rook.x = self.king.x + 1
            self.board[7][self.king.x + 2] = self.king
            self.king.x += 2
            # draw over previous king and rook positions
            self.draw_tile(screen, (7, 7))
            self.draw_tile(screen, (self.king.x - 2, 7))
        # draw new positions of king and rook
        self.draw_tile(screen, (rook.x, rook.y))
        self.draw_tile(screen, (self.king.x, self.king.y))
